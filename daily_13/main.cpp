...
// Note: If we used set instead of unordered_set, the complexity for insert and find would be O(logN) instead of O(1). Since we need no ordering, unordered set is preferred.


int main()
{

    vector< std::pair<string, uint> > test_vector;
    test_vector.push_back( make_pair<string,int>("abcbbbcccba", 1u) );  // Expected:   3 (bbb) or (ccc)
    test_vector.push_back( make_pair<string,int>("abcbbbcccba", 2u) );  // Expected:   9 (bcbbbcccb)
    test_vector.push_back( make_pair<string,int>("abcbbbcccba", 3u) );  // Expected:  11 (abcbbbcccba)

    test_vector.push_back( make_pair<string,int>("DDDabcxDDDxyzxyzxyzdd", 2u) );  // Expected:  5 (xDDDx)
    test_vector.push_back( make_pair<string,int>("DDDabcxDDDxyzxyzxyzdd", 3u) );  // Expected:  9 (xyzxyzxyz)
    test_vector.push_back( make_pair<string,int>("DDDabcxDDDxyzxyzxyzdd", 4u) );  // Expected: 13 (xDDDxyzxyzxyz)
    ...
    for(int i=0; i<test_vector.size(); i++)
    {
        ...
        uint longest_substring = lengthOfLongestSubstring(str, k);
