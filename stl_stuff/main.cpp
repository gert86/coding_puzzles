...
// Helper method to print a vector of doubles
string printVec(const vector<double> &vec)
{
    stringstream ss;
    ss << "[";
    for (int i=0; i<vec.size(); i++)
...
//    sort         // Normal sort
    ...
    n = sizeof(myints)/sizeof(myints[0]);
    ...
    //    --------------------------
    //    Most important functions for vector, stack, queue, deque, priority_queue, forward_list, list, unordered_map, unordered_set, map, set
    ...
    #define PPS print_lambda_list(my_forward_list)
    cout << endl << "forward_list " << endl;
    std::forward_list<int> my_forward_list = {5};
    my_forward_list.push_front(4);                                                                  // only push_front
    my_forward_list.push_front(3); my_forward_list.push_front(2); PPS;                              // 2, 3, 4, 5
    my_forward_list.insert_after(my_forward_list.before_begin(), 1); PPS;                           // 1, 2, 3, 4, 5
    my_forward_list.erase_after(my_forward_list.before_begin()); PPS;     // erase - by iterator    // 2, 3, 4, 5
    my_forward_list.remove(4); PPS;                                       // remove - by value      // 2, 3, 5
    accessed_val = my_forward_list.front();                               // access only to front()
    my_forward_list.pop_front(); PPS;                                     // pop is only for front  // 3, 5
    ...
    #define PPD print_lambda_list(my_list)
    cout << endl << "list " << endl;
    std::list<int> my_list = {3};
    my_list.push_front(2);                                                                                  // push_front() or push_back()
    my_list.push_back(4);  my_list.push_back(5);   PPD;                                                     // 2, 3, 4, 5
    my_list.insert(my_list.begin(), 1);  PPD;                                                               // 1, 2, 3, 4, 5
    my_list.erase(my_list.begin()); PPD;                                    // erase - by iterator          // 2, 3, 4, 5
    my_list.remove(4); PPD;                                                 // remove - by value            // 2, 3, 5
    accessed_val = my_list.front(); accessed_val = my_list.back();          // access to front() or back()
    my_list.pop_front(); PPD;                                               // pop from front or back       // 3, 5
    my_list.pop_back(); PPD;                                                // pop from front or back       // 3
    my_list.clear();
    // no find, but broad API (reverse(), sort(), unique(), ...)
